...
public class ScheduleDAO {
    ...
    public Schedule getScheduleById(int id) throws SQLException {
        ...
        try (Connection con = DBConnection.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    schedule = new Schedule(rs.getInt("schedule_id"), rs.getInt("room_id"),
                            rs.getString("semester"), rs.getString("class"), rs.getBoolean("occupied"));
                }
            }
        }
        return schedule;
    ...
    public List<Schedule> searchSchedules(int capacity, boolean hasProjector) throws SQLException {
        String query = "SELECT s.* FROM Schedule s JOIN Room r ON s.room_id = r.room_id " +
                "WHERE r.capacity >= ? AND r.has_projector = ?";
        ...
        try (Connection con = DBConnection.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    Schedule schedule = new Schedule(rs.getInt("schedule_id"), rs.getInt("room_id"),
                            rs.getString("semester"), rs.getString("class"), rs.getBoolean("occupied"));
